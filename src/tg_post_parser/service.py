...
from .config import AnalysisConfig, SourceConfig
...
class PostProcessor:
    ...
    async def process(self, post: IncomingPost, source: SourceConfig) -> ProcessedPost | None:
        ...
        if self.analysis.enabled:
            ...
            analysis = await self.rewriter.analyze(post.text, post.image_paths, history)
            if analysis.is_duplicate or analysis.is_advertisement:
                status = "filtered_advertisement" if analysis.is_advertisement else "filtered_duplicate"
